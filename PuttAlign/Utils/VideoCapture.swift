...
class VideoCapture: NSObject {
    ...
    private let sessionQueue = DispatchQueue(
        label: "com.rollyk.estimating-human-pose-with-posenet.sessionqueue")
    
    static let shared: VideoCapture = {
        let instance = VideoCapture()
        // setup code
        return instance
    }()
    
    public func setBias(bias: Float) {
        guard let device = AVCaptureDevice.default(
            ...
            do {
                try device.lockForConfiguration()
                    ...
                    // Handle errors here
                    print("There was an error setting exposure for the device's camera")
                }
    }
    
    public func getTargetBias() -> (min:Float, max:Float, current:Float) {
    ...
    /// Does autoexposure
    public func setExposure(forPoint point: CGPoint) {
        guard let device = AVCaptureDevice.default(
            .builtInWideAngleCamera,
            for: AVMediaType.video,
            position: cameraPostion) else {
    ...
    private func setCaptureSessionOutput() throws {
        ...
        DispatchQueue.main.sync {
            ...
            self.orientation = AVCaptureVideoOrientation(deviceOrientation: interaface!)
        }
