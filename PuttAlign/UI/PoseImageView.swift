...
class PoseImageView: UIImageView {
    ...
    let graphics: UIImageView = UIImageView(frame: CGRect.zero)
    ...
    @IBInspectable var eyeColor: UIColor = UIColor.systemTeal
    @IBInspectable var bodyColor: UIColor = UIColor.systemTeal
    ...
    override func layoutSubviews() {
        ...
        addSubview(self.graphics)
        self.graphics.frame = frame
    }
    ...
    func show(poses: [Pose], with frame: CGImage) {
        let dstImageSize = CGSize(width: frame.width, height: frame.height)
        let dstImageFormat = UIGraphicsImageRendererFormat()
        ...
        let dstImage = renderer.image { rendererContext in
            ...
            for pose in poses {
                ...
                for joint in pose.joints.values.filter({ $0.isValid }) {
                    if(joint.name == .leftEye || joint.name == .rightEye || joint.name == .leftShoulder || joint.name == .rightShoulder)
                    {
    ...
    func drawLine(from parentJoint: Joint,
        ...
        var lineColor = bodyColor
        if parentJoint.name.rawValue < 3 {
            lineColor = eyeColor
        }
        cgContext.setStrokeColor(lineColor.cgColor)
        cgContext.setLineWidth(segmentLineWidth)
        
        let x1 = parentJoint.position.x
        let y1 = parentJoint.position.y
        let x2 = childJoint.position.x
        let y2 = childJoint.position.y
        ...
        let px2 = x2 - ext
        let py2 = px2 * slope + b
        let startP = CGPoint(x: px1, y: py1)
        let endP = CGPoint(x: px2, y: py2)
        
        cgContext.move(to: startP)
        cgContext.addLine(to: endP)
